...
pub fn rescaler(
    ...
    loop {
        ...
        spawn_send_thread(my_process_index, new_worker_index, stream.try_clone().unwrap(), log_sender.clone(), network_promise).expect("Spawn send thread");
        spawn_recv_thread(my_process_index, new_worker_index, stream.try_clone().unwrap(), log_sender.clone(), network_future, threads).expect("Spawn recv thread");
        ...
        rescaler_txs
            ...
            .zip(bootstrap_addrs)
            .for_each(|((((tx, buzzer), promise), future), bootstrap_addr)| {
                ...
                // need to buzz the rx worker thread, in case it's parked
                buzzer.buzz();
            });
    }
}
...
fn recv_bootstrap_addr(mut stream: &mut TcpStream, my_process_index: usize, threads: usize) -> Vec<Option<SocketAddrV4>> {
    ...
    (0..threads).map(|worker_offset|{
        let worker_index = my_process_index*threads + worker_offset;
        if bootstrap_server_index == worker_index { Some(SocketAddrV4::new(Ipv4Addr::from(ip), port)) }
        else { None }
    }).collect()
}
